#!/bin/bash
#
#  web connection wrapper and working example for remote.it P2P Connections
#
#  rweb.sh <-v> <-v> servicename
#
#  <optional>  -v = verbose -v -v =maximum verbosity
#
#  will store info in ~/.remoteit/
#
#  License See : https://github.com/remoteit/ssh_client
#
...
rweb.sh - A web connection wrapper for remote.it showing use of the client-side daemon for a P2P connection.
------------------------------------------
This software allows you to make P2P connections to your remote.it enabled http or https servers.
...
To get a list of all http and https services associated with your account, use:

./rweb.sh -l

To make an web connection to any given service, use:

./rweb.sh service-name

service-name is the remote.it name you gave to this device's web connection.
...
./rweb.sh -v service-name

./rweb.sh -v -v service-name
...
From there we parse the JSON output of the device list and find the entry corresponding to the device name you gave.  We find the UID (JSON ["deviceaddress"]) for this entry and use this in conjunction with the remote.it daemon (connectd) in client mode to initiate a peer to peer connection.
...
Now you have a listener at 127.0.0.1:33000 that allows a connection to your remote device's http service.
...
#
echo "remote.it rweb.sh Version $VERSION $MODIFIED"
create_config
